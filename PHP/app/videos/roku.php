...
else if ($action == "link")
{
	...
	print getLink($deviceID, $regCode);
...
function getLink($deviceID, $code)
{
	$roku = new Roku();

	$token = $roku->getLink($deviceID, $code);
...
class Roku
{
	...
	function getLink($deviceID, $code)
	{
		...
		try
		{
				$sql = "select token from roku_reg where device_id=? and gen_code=?";
				$q = $this->db->prepare($sql);
				$q->execute(array($deviceID,$code));
				$row = $q->fetch();	
