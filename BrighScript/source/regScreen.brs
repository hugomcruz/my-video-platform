...
Function doRegistration() As Integer
   ...
   'Success case: getRegResult_success (always returns success)
    ...
    m.UrlBase         = "http://app.berzuk.com/app/videos/roku"
    m.UrlGetRegCode   = m.UrlBase + "/code"
    m.UrlGetRegResult = m.UrlBase + "/link"
    m.UrlWebSite      = "http://app.berzuk.com/roku/pin/videos/"
...
Function getRegistrationCode(sn As String) As String
    ...
    for each e in xml.GetBody()
        ...
        else if e.GetName() = "retryInterval" then
            retryInterval = strtoi(e.GetBody())
        else if e.GetName() = "retryDuration" then
